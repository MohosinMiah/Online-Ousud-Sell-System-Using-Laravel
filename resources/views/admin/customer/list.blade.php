...
if (!empty($all_customer_lists)) { ?>
    ...
    <table id="datatable-buttons" class="table table-striped table-bordered dataTable no-footer dtr-inline" role="grid" aria-describedby="datatable-buttons_info" style="width: 972px;">
                      <thead>
                        ...
                        <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1" colspan="1" style="width: 56px;" aria-label="City: activate to sort column ascending">City</th>
                     ...
                     <tr role="row" class="odd">
                        ...
                        <td tabindex="0" class="sorting_1">{{$all_customer_list->customer_id}}</td>

                          <td><a href="/customer/details/{{$all_customer_list->customer_id}}" title="Show Details">{{ $all_customer_list->name }} </a> </td>
                          <td>{{ $all_customer_list->phone }}  </td>
                          <td>{{$all_customer_list->city}} </td>
